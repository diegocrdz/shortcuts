 ...
 */
use crate::shortcuts::{
    extract_icon_from_exe, game_shortcut, launcher_shortcut, Shortcut, SOURCE_EPIC,
};
...
pub fn scan_epic(app: &AppHandle, excluded_ids: &HashSet<String>) -> Vec<Shortcut> {
    ...
    for entry in entries.flatten() {
        ...
        let (Some(name), Some(app_name)) =
            (manifest.display_name.clone(), manifest.app_name.clone())
        else {
            continue;
        };
